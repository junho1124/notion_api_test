...
class CalenderViewModel extends ChangeNotifier {
  ...
  Future<Result> makeEvent(String token, String db) async {
    final _calenderItems = await DataRepository().getTodoItems(token, db);
    if(_calenderItems is Success<List<Data>>) {
      final List<NeatCleanCalendarEvent> resultList = _calenderItems.data
          .map((e) => NeatCleanCalendarEvent(e.name,
              ...
              endTime: DateTime(e.endDate.year, e.endDate.month, e.endDate.day,
                  e.endDate.hour, e.endDate.minute),
